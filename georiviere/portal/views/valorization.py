...
from georiviere.portal.serializers.valorization import (
...
class POIViewSet(GeoriviereAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(portals__id=portal_pk).annotate(
    ...
    @action(
        ...
        methods=["get"],
        permission_classes=[],
        url_path=r"category/(?P<category_pk>\d+)",
        url_name="category",
    )
    def category(self, request, *args, **kwargs):
        ...
        qs = self.filter_queryset(
            self.get_queryset().filter(type__in=category.types.all())
        )
