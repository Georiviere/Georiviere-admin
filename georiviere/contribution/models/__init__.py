...
class Contribution(
    ...
    WatershedPropertiesMixin,
    ZoningPropertiesMixin,
    AddPropertyBufferMixin,
    MapEntityMixin,
):
    """contribution model"""
    ...
    name_author = models.CharField(
        max_length=128, verbose_name=_("Name author"), blank=True
    )
    first_name_author = models.CharField(
        max_length=128, verbose_name=_("First name author"), blank=True
    )
    email_author = models.EmailField(verbose_name=_("Email"))
    date_observation = models.DateTimeField(
        editable=True, verbose_name=_("Observation's date")
    )
    ...
    description = models.TextField(
        ...
        help_text=_("Description of the contribution"),
        blank=True,
    )
    ...
    def __str__(self):
        ...
        if hasattr(self, "potential_damage"):
            return (
                f"{self.email_author} {ContributionPotentialDamage._meta.verbose_name.title()} "
                f"{self.potential_damage.get_type_display()}"
            )
        elif hasattr(self, "fauna_flora"):
            return (
                f"{self.email_author} {ContributionFaunaFlora._meta.verbose_name.title()} "
                f"{self.fauna_flora.get_type_display()}"
            )
        elif hasattr(self, "quality"):
            return (
                f"{self.email_author} {ContributionQuality._meta.verbose_name.title()} "
                f"{self.quality.get_type_display()}"
            )
        elif hasattr(self, "quantity"):
            return (
                f"{self.email_author} {ContributionQuantity._meta.verbose_name.title()} "
                f"{self.quantity.get_type_display()}"
            )
        elif hasattr(self, "landscape_element"):
            return (
                f"{self.email_author} {ContributionLandscapeElements._meta.verbose_name.title()} "
...
class ContributionPotentialDamage(models.Model):
    ...
    landing_type = models.ForeignKey(
        LandingType, on_delete=models.PROTECT, null=True, verbose_name=_("Landing type")
    )
    excessive_cutting_length = models.FloatField(
        default=0.0,
        null=True,
        blank=True,
    ...
    length_bank_erosion = models.FloatField(
        default=0.0,
        null=True,
        blank=True,
        verbose_name=_("Length bank erosion (in meters)"),
        help_text=_(
            "Distance between the foot of the bank and the foot of " "the erosion."
        ),
    )
    bank_height = models.FloatField(
        default=0.0,
        null=True,
        blank=True,
        verbose_name=_("Bank height (in meters)"),
        help_text=_(
            "Bank height (measured between the foot of the bank and the top "
...
class ContributionFaunaFlora(models.Model):
    ...
    invasive_species = models.ForeignKey(
        InvasiveSpecies, on_delete=models.PROTECT, null=True
    )
    number_heritage_species = models.IntegerField(
        default=0, null=True, blank=True, verbose_name=_("Number heritage species")
    )
    heritage_species = models.ForeignKey(
        HeritageSpecies, on_delete=models.PROTECT, null=True
    )
    heritage_observation = models.ForeignKey(
        HeritageObservation, on_delete=models.PROTECT, null=True
    )
    number_fish_species = models.IntegerField(
        default=0, null=True, blank=True, verbose_name=_("Number fish species")
    )
...
class ContributionQuality(models.Model):
    ...
    nature_pollution = models.ForeignKey(
        NaturePollution, on_delete=models.PROTECT, null=True
    )
    type_pollution = models.ForeignKey(
        TypePollution, on_delete=models.PROTECT, null=True
    )
    contribution = models.OneToOneField(
        Contribution, parent_link=True, on_delete=models.CASCADE, related_name="quality"
    )
...
class CustomContributionType(models.Model):
    ...
    def get_json_schema_form(self):
        linked_fields = self.fields.all()
        fields = {}
        for field in self.fields.all():
            fields[field.key] = field.get_field_schema()
        return {
            # "title": self.label,
            "type": "object",
            "properties": fields,
...
class CustomContributionTypeField(models.Model):
    ...
    key = models.SlugField(
        max_length=150,
        verbose_name=_("Key"),
        help_text=_("Key used in JSON data field."),
        editable=False,
    )
    ...
    def get_customization_json_schema_form(self):
        base_schema = {
            "type": "object",
            "properties": {},
        }
        ...
        if self.value_type == self.FieldTypeChoices.TEXT:
            base_schema["properties"] = {
                "placeholder": {
                    "type": "string",
                    "title": _("Placeholder"),
        ...
        if self.value_type == self.FieldTypeChoices.INTEGER:
            base_schema["properties"] = {
                "minimum": {
                    "type": "integer",
                    "title": _("Min. value"),
        ...
        if self.value_type == self.FieldTypeChoices.FLOAT:
            base_schema["properties"] = {
                "minimum": {
                    "type": "number",
                    "title": _("Min. value"),
        ...
        if self.value_type == self.FieldTypeChoices.BOOLEAN:
            base_schema["properties"] = {
                "widget": {
                    "type": "string",
                    "choices": [
    ...
    def get_field_schema(self):
        ...
        base_schema.update(self.options)

        if self.value_type == self.FieldTypeChoices.TEXT:
            base_schema["widget"] = "textarea"
        elif self.value_type == self.FieldTypeChoices.INTEGER:
            base_schema["type"] = "integer"
        elif self.value_type == self.FieldTypeChoices.FLOAT:
            base_schema["type"] = "number"
        elif self.value_type == self.FieldTypeChoices.BOOLEAN:
            base_schema["type"] = "boolean"
        elif self.value_type == self.FieldTypeChoices.DATE:
            base_schema["format"] = "date"
